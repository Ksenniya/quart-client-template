...
# Workflow function to process and enrich the search job entity
async def process_company_search_job(entity):
    # The entity already contains the initial job data.
    # The original search parameters are stored under "payload" key.
...
async def search_companies(data: SearchRequest):
    ...
    # Include search payload in the job data so that the workflow function has access to it.
    job_data = {
        "status": "processing",
    ...
    # The workflow function process_company_search_job is applied asynchronously before persistence.
    job_id = await entity_service.add_item(
        token=cyoda_token,
        entity_model=ENTITY_MODEL,
        entity_version=ENTITY_VERSION,  # always use this constant
        entity=job_data,  # the job data including search parameters
