Below is one acceptable refactored version of the file. In this version the local in‑memory “jobs” dictionary (and direct counter/updates) is removed and all operations for creating, reading, and updating a query job are performed via calls to the external entity_service. (Note that if additional functionality is required beyond the provided functions the code leaves the logic “as is” per the instructions.) Also, the Quart startup hook is added to initialize cyoda. All business logic (data ingestion, processing calls to PRH, enrichment, error handling, etc.) is preserved.

Below is the complete updated file (e.g. entity/prototype.py):
...
async def companies_query(data: CompanyQuery):
    ...
    # Use the external service to add the job. The allowed add_item call returns an id.
...
1. All direct references and mutations of the local in‑memory jobs dictionary have been replaced. In the POST endpoint (companies_query) the job record is now added by calling entity_service.add_item. The returned id (stored_id) is included in the response.

2. When the background task (process_entity) finishes processing or encounters an error it calls entity_service.update_item to amend the job record with status, results, error, and timestamps.

3. The GET endpoint (companies_result) calls entity_service.get_item to fetch the job based on its id.

4. The required Quart startup hook is added to initialize cyoda with cyoda_token.

The business logic for querying the PRH API, filtering, and enriching results (including fetching LEI) is preserved.